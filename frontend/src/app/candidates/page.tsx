...
export default function DashboardPage() {
  ...
  const loadData = async (nextFilters: SearchParams) => {
    ...
    const [candidateData, analyticsData] = await Promise.all([
      apiGet<Candidate[]>(
        `${buildCandidateQuery(nextFilters)}${buildCandidateQuery(nextFilters).includes("?") ? "&" : "?"}include_deleted=${showTrash ? "true" : "false"}`,
      ),
  ...
  const canManageCandidate = (c: Candidate) => {
    const ownerEmail = String(
      (c.parsed_json as any)?.owner_email || "",
    ).toLowerCase();
    ...
    return (
      ...
      (!!ownerUserId && !!meId && ownerUserId === meId)
    );
  };
  ...
  const bulkSetStatus = async (status: CandidateStatus) => {
    ...
    try {
      await Promise.all(
        selectedIds.map((id) =>
          apiPatch<Candidate>(`/api/candidates/${id}`, { status }),
        ),
      );
    ...
    <div className="grid page-enter">
      <div className="toolbar">
        ...
        <div className="toolbar-actions">
          ...
          <a
            className="report-btn report-csv"
            href={apiUrl("/api/reports/candidates.csv")}
          >
            {t("report_candidates_csv")}
          </a>
          <a
            className="report-btn report-analytics"
            href={apiUrl("/api/reports/analytics.csv")}
          >
            {t("report_analytics_csv")}
          </a>
          <a
            className="report-btn report-xlsx"
            href={apiUrl("/api/reports/reports.xlsx")}
          >
            {t("report_xlsx")}
          </a>
          <a
            className="report-btn report-pdf"
            href={apiUrl("/api/reports/report.pdf")}
          >
            {t("report_pdf")}
          </a>
        </div>
      ...
      <div className="card">
        <div className="toolbar">
          ...
          <div className="toolbar-actions">
            <Link className="chip" href="/upload">
              + Upload CVs
            </Link>
            <Link className="chip" href="/jobs">
              Run Matching
            </Link>
            <Link className="chip" href="/pipeline">
              Open Pipeline
            </Link>
          </div>
        </div>
        ...
        <>
          {loadingPage ? (
            <div className="card">
              <div
                className="skeleton"
                style={{ height: 20, width: 220, marginBottom: 10 }}
              />
          ...
          <div className="grid grid-4">
            ...
            <div className="card stat-card">
              ...
              <small>
                {analytics.source_effectiveness?.[0]?.share_pct || 0}%
              </small>
            </div>
          </div>
          ...
          <div className="grid grid-3">
            ...
            <div className="card">
              ...
              <ul>
                {analytics.source_hire_effectiveness.map((s) => (
                  <li key={s.source}>
                    {s.source}: {s.hired}/{s.total} (
                    <strong>{s.hire_rate_pct}%</strong>)
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="card">
        <div className="toolbar sticky-toolbar">
          <h3>
            {showTrash ? "Deleted Candidates" : "Candidates"} (
          ...
          <div className="toolbar-actions">
            ...
            <button
              className="btn-outline"
              type="button"
              onClick={() => setSelectedIds(pagedCandidates.map((c) => c.id))}
            >
              Select Page
            </button>
            <button
            ...
            <button
              type="button"
              onClick={() => bulkSetStatus("screening")}
            ...
            <button
              type="button"
              onClick={() => bulkSetStatus("interview")}
              disabled={busy || !selectedIds.length}
            >
              Move Interview
            </button>
        ...
        <div className="candidate-table-scroll">
          <table
            className={`candidate-table ${compactMode ? "candidate-table-compact" : ""}`}
          >
            <thead>
              <tr>
                <th></th>
                ...
                <tr>
                  <td colSpan={8}>
                    <div className="empty-state">
                      ...
                      <small>
                        Try changing filters, importing CVs, or switching
                        Active/Trash.
                      </small>
                    </div>
                  </td>
                ...
                <tr key={c.id}>
                  ...
                  <td>
                    <select
                      ...
                      onChange={(e) =>
                        updateOneStatus(c.id, e.target.value as CandidateStatus)
                      }
                    >
                      {STATUS_OPTIONS.map((s) => (
                        <option key={s} value={s}>
        ...
        <div className="toolbar" style={{ marginTop: 12 }}>
          <small>
            Page {page} / {totalPages} · Total {sortedCandidates.length}{" "}
            candidates
          </small>
          <div className="toolbar-actions">
            <button
