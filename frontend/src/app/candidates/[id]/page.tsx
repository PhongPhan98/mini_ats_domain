...
function toForm(c: Candidate): CandidateForm {
  return {
    ...
    summary: c.summary || "",
    domain_tags_text: String(
      ((c.parsed_json as any)?.domain_tags || []).join(", "),
    ),
    experience_details_text: String(
      ((c.parsed_json as any)?.experience_details || []).join("\n"),
    ),
...
function formatTimelineEvent(lang: string, ev: TimelineEvent) {
  ...
  if (type === "created")
    return vi ? "Tạo hồ sơ ứng viên" : "Candidate profile created";
  if (type === "status") {
  ...
  if (type === "automation") {
    ...
    return vi
      ? `Tự động hoá đã gửi thông báo cho vòng ${stage}`
      : `Automation sent notification for stage ${stage}`;
  }
  if (type === "comment") return vi ? `Bình luận: ${val}` : `Comment: ${val}`;
  if (type === "mention")
    return vi ? `Đã nhắc thẻ: ${val}` : `Mentioned: ${val}`;
  if (type === "note") {
    if (val.startsWith("auto_action:notify_on_stage_change:")) {
      const stage = val.split(":").pop() || val;
      return vi
        ? `Tự động gửi thông báo khi ứng viên vào vòng ${stage}`
        : `Auto notification sent when candidate entered ${stage} stage`;
    }
    if (val.startsWith("status_changed:")) {
      const st = val.split(":").pop() || val;
      return vi ? `Cập nhật trạng thái: ${st}` : `Status updated: ${st}`;
    }
    if (val === "candidate_soft_deleted")
      return vi
        ? "Ứng viên được chuyển vào Thùng rác"
        : "Candidate moved to Trash";
    if (val === "candidate_restored")
      return vi ? "Ứng viên được khôi phục" : "Candidate restored";
    return vi ? `Ghi chú: ${val}` : `Note: ${val}`;
  }

  if (type === "share") {
    if (val.startsWith("shared_with:"))
      return vi
        ? `Đã chia sẻ cho ${val.replace("shared_with:", "")}`
        : `Shared with ${val.replace("shared_with:", "")}`;
    if (val.startsWith("unshared_with:"))
      return vi
        ? `Đã huỷ chia sẻ với ${val.replace("unshared_with:", "")}`
        : `Unshared with ${val.replace("unshared_with:", "")}`;
  }
  return val;
}
...
export default function CandidateDetailPage({
  ...
  const [activeTab, setActiveTab] = useState<
    "profile" | "interviews" | "notes" | "timeline"
  >("profile");
  const [emailMode, setEmailMode] = useState<"interview" | "rejection" | null>(
    null,
  );
  ...
  const loadComments = async (id: string) => {
    const data = await apiGet<CandidateComment[]>(
  ...
  const loadScorecards = async (id: string) => {
    const data = await apiGet<InterviewScorecard[]>(
      `/api/candidates/${id}/scorecards`,
    );
    setScorecards(data);
  };

  const loadSchedules = async (id: string) => {
    const data = await apiGet<InterviewSchedule[]>(
      `/api/candidates/${id}/schedules`,
    );
    setSchedules(data);
  };
  ...
  const isOwner =
    ...
    (!!me?.id && ownerUserId > 0 && Number(me.id) === ownerUserId);
  const isViewOnly = !isOwner;
  const pendingOwnershipRequests = (
    ((candidate?.parsed_json as any)?.ownership_requests || []) as any[]
  ).filter(
    (r) =>
      r.status === "pending" &&
      String(r.to_email || "").toLowerCase() === ownerEmail,
  );
  ...
  const onAddComment = async () => {
    ...
    if (!body || !candidateId) {
      notify(t("missing_required_fields"), "error");
      return;
    }
  ...
  const onShareCandidate = async () => {
    if (!candidateId || !shareEmail.trim()) {
      notify(t("missing_required_fields"), "error");
      return;
    }
    await apiPost(`/api/candidates/${candidateId}/share`, {
  ...
  const onOwnershipDecision = async (
    requestId: string,
    decision: "approve" | "reject",
  ) => {
    if (!candidateId) return;
    await apiPost(
      `/api/candidates/${candidateId}/ownership/requests/${requestId}/decision`,
      { decision },
    );
    const updated = await apiGet<Candidate>(`/api/candidates/${candidateId}`);
    setCandidate(updated);
  ...
  const onScheduleInterview = async () => {
    if (!candidateId || !schedInterviewer || !schedAt) {
      notify(t("missing_required_fields"), "error");
      return;
    }
    ...
    <div className="grid page-enter">
      <div className="card">
        {!isOwner ? (
          ...
          >
            View-only access: you can comment and request ownership; only owner
            can edit candidate profile.
          </div>
        ) : null}
        <div className="toolbar">
          <div>
            ...
            <h2 style={{ margin: "10px 0 0" }}>
              {candidate.name || t("candidate_detail")}
            </h2>
          ...
          {!isOwner ? (
            <div className="toolbar-actions">
              <input
                ...
                placeholder="Reason to request ownership"
              />{" "}
              <button
                ...
                onClick={async () => {
                  await apiPost(
                    `/api/candidates/${candidateId}/ownership/request`,
                    { reason: ownershipReason },
                  );
                  notify("Ownership request sent", "success");
                ...
                disabled={(
                  ((candidate?.parsed_json as any)?.ownership_requests ||
                    []) as any[]
                ).some(
                  (r) =>
                    String(r.from_email || "").toLowerCase() === meEmail &&
                    r.status === "pending",
                )}
              >
                Request ownership
        ...
        <div className="toolbar">
          <div>
            ...
            <small style={{ display: "block" }}>
              Stage control and quick actions
            </small>
          </div>
          <div className="toolbar-actions">
            <select
            ...
            {!isViewOnly ? (
              <button
                ...
                style={{ width: "auto" }}
                onClick={() => {
                  setEmailMode("interview");
                  setEmailTo(candidate?.email || "");
                  setEmailSubject(
                    `Interview Invitation - ${candidate?.name || "Candidate"}`,
                  );
                  setEmailBody(
            ...
            {!isViewOnly ? (
              <button
                ...
                style={{ width: "auto" }}
                onClick={() => {
                  setEmailMode("rejection");
                  setEmailTo(candidate?.email || "");
                  setEmailSubject(
                    `Application Update - ${candidate?.name || "Candidate"}`,
                  );
                  setEmailBody(
        ...
        <div className="card">
          {!isOwner ? (
            <div
              ...
              style={{ marginBottom: 10, display: "inline-flex" }}
            >
              View-only access: you can comment and request ownership; only
              owner can edit candidate profile.
            </div>
          ) : null}
          <div className="toolbar">
        ...
        <div id="sec-profile" className="card">
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div className="chip-wrap" style={{ marginTop: 8 }}>
              {normalizeCommaList(form.skills_text)
                .slice(0, 12)
                .map((sk) => (
              ...
              {!normalizeCommaList(form.skills_text).length ? (
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <textarea
              ...
              onChange={(e) =>
                updateField("previous_companies_text", e.target.value)
              }
              readOnly={isViewOnly}
              disabled={isViewOnly}
            />
          ...
          <div className="grid grid-2" style={{ marginTop: 12 }}>
            <div>
              ...
              <input
                value={form.domain_tags_text || ""}
                onChange={(e) =>
                  updateField("domain_tags_text", e.target.value)
                }
                readOnly={isViewOnly}
                disabled={isViewOnly}
              />
            ...
            <div className="timeline" style={{ marginTop: 12 }}>
                ...
                <div className="timeline-item" key={s.id}>
                  ...
                  <div>
                    <div>
                      ...
                      {t("with_label")} {s.interviewer_email}
                    </div>
                    <small>
              ...
              {!schedules.length && (
                <div className="empty-state">
                  ...
                  <small>
                    Use the form above to schedule the first interview.
                  </small>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-2">
              ...
              <div>
                ...
                <input
                  ...
                  max={5}
                  value={scoreProblem}
                  onChange={(e) => setScoreProblem(e.target.value)}
                />
              </div>
              <div>
            ...
            <div className="timeline" style={{ marginTop: 12 }}>
              {scorecards.map((s) => (
                <div className="timeline-item" key={s.id}>
                  ...
                  <div>
                    <div>
                      Stage: <strong>{s.interview_stage}</strong> | Overall:{" "}
                      <strong>{s.overall_score ?? "-"}</strong>
                    </div>
                    <small>
                      Tech {s.criteria_scores?.technical ?? "-"}, Comm{" "}
                      {s.criteria_scores?.communication ?? "-"}, Problem{" "}
              ...
              {!scorecards.length && (
                <div className="empty-state">
                  ...
                  <small>Submit first scorecard after interview.</small>
                </div>
              )}
            </div>
          </div>
      ...
      {activeTab === "notes" ? (
        <div id="sec-comments" className="grid grid-2">
          <div className="card">
            ...
            <small>
              Team notes (chat-style). Mention teammates with @name.
            </small>
            ...
            <div className="timeline" style={{ marginTop: 12 }}>
              {comments.map((c) => (
                <div className="timeline-item comment-item" key={c.id}>
                  ...
                  <div>
                    <div>
                      <strong>
                        {c.author_name || `User #${c.author_user_id}`}
                      </strong>
                    </div>
              ...
              {!comments.length && (
                <div className="empty-state">
                  ...
                  <small>Add first note to start collaboration.</small>
                </div>
              )}
            </div>
          </div>
          ...
          {!isViewOnly && isOwner ? (
            <div id="sec-collab" className="card">
              <h3>Ownership Requests Inbox</h3>
              <small>
                Approve or reject transfer requests from other HR members.
              </small>
              <div className="timeline" style={{ marginTop: 10 }}>
                {pendingOwnershipRequests.map((r: any) => (
                  <div id={`req-${r.id}`} key={r.id} className="timeline-item">
                    ...
                    <div>
                      <div>
                        <strong>{r.from_email}</strong> requested ownership
                      </div>
                      {r.reason ? <small>Reason: {r.reason}</small> : null}
                      <small>{r.created_at}</small>
                      <div className="toolbar-actions" style={{ marginTop: 6 }}>
                        <button
                          style={{ width: "auto" }}
                          onClick={() => onOwnershipDecision(r.id, "approve")}
                        >
                          Approve
                        </button>
                        <button
                          className="btn-outline"
                          style={{ width: "auto" }}
                          onClick={() => onOwnershipDecision(r.id, "reject")}
                        >
                          Reject
                        </button>
                      </div>
                ...
                {!pendingOwnershipRequests.length && (
          ...
          {!isViewOnly && isOwner ? (
            <div id="sec-collab-share" className="card">
              <h3>Team Collaboration Sharing</h3>
              <small>
                Send sharing invitation to another HR. They can approve to
                create their own cloned candidate record.
              </small>
              <div className="toolbar-actions" style={{ marginTop: 8 }}>
                ...
                />
                <button style={{ width: "auto" }} onClick={onShareCandidate}>
                  Send invite
                </button>
              </div>
              <div className="chip-wrap" style={{ marginTop: 10 }}>
                {(
                  (candidate.parsed_json as any)?.collaborator_emails || []
                ).map((em: string) => (
                ...
                {!((candidate.parsed_json as any)?.collaborator_emails || [])
                  .length && (
                  <small>
                    No direct collaborators yet (invite-based clone flow
                    enabled).
                  </small>
                )}
              </div>
          ...
          {!isViewOnly ? (
            <div id="sec-timeline" className="card">
              ...
              <div className="timeline timeline-scroll">
                  ...
                  >
                    ...
                    <div>
                      <div className="timeline-title">
                        {formatTimelineTitle(lang, event.type)}
                      </div>
                ...
                {!timelineOf(candidate).length && (
                  <div className="empty-state">
                    <strong>No activity yet</strong>
                    <small>Actions on this candidate will appear here.</small>
                  </div>
                )}
              </div>
            </div>
      ...
      {emailMode ? (
        <div className="modal-overlay" onClick={() => setEmailMode(null)}>
            ...
            <div className="toolbar-actions" style={{ marginTop: 10 }}>
              ...
              <button
                ...
                onClick={async () => {
                  const path =
                    emailMode === "interview"
                      ? `/api/candidates/${candidateId}/email/interview`
                      : `/api/candidates/${candidateId}/email/rejection`;
                  await apiPost(path, {
                    to_email: emailTo,
